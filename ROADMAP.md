# StockOverflow Roadmap v2

> 一站式智能股票分析平台 — 让数据驱动决策

---

## 已完成 ✅

### 核心功能
- [x] 股票搜索 + 5 年历史日线数据 + 增量更新
- [x] 16 项技术指标自动计算（MACD, BB, Ichimoku, TD, RSI, Stoch, ADX, VWAP, ATR, OBV, CCI, %R, SAR, MFI, Fib, CNN F&G）
- [x] 指标叠加绘制（价格叠加 + 子图同步）
- [x] 因子表达式引擎（20+ 内置函数，支持 and/or 组合）
- [x] 回测引擎（事件驱动，止损/止盈/手续费/滑点）
- [x] 纸盘交易系统（限价单/止损单/止盈单）
- [x] 多策略对比 + 参数敏感性 + Monte Carlo + Walk-Forward
- [x] CNN 恐慌指数 + 多源新闻（yfinance + Finviz）+ 新闻情绪
- [x] 财报数据 + 机构持仓 + 期权链 + 分红历史
- [x] Watchlist 自选股 + Dashboard 首页
- [x] 股票对比功能
- [x] 数据备份（手动 + 启动自动）

### LLM 集成
- [x] OpenAI 兼容 API（6 个模型预设）
- [x] 多轮对话 + 对话历史持久化
- [x] SSE 流式输出
- [x] 结构化预测（BUY/SELL/HOLD + 价格 + 止损 + 置信度）
- [x] 多空对比 + 定投建议 + 策略解释 + 学习路径
- [x] 图表标注（支撑/阻力位）

### 基础设施
- [x] SQLite WAL 模式 + LRU 缓存 + 请求队列
- [x] 统一错误处理 + 结构化日志
- [x] .env 配置 + Docker + Railway/Fly.io 部署
- [x] 66 个测试通过
- [x] WebSocket 实时价格流
- [x] 多时间周期（1D/日/周/月/季/年）

---

## 🔥 当前重点 — 待解决的核心问题

### 1. 图表稳定性
- [ ] **修复 lightweight-charts 渲染错误**：无限缩放、跨时间周期切换时的崩溃
- [ ] **统一时间格式处理**：日线用日期字符串，分时用 Unix 时间戳，切换时正确处理
- [ ] **图表生命周期管理**：切换股票/时间周期时正确销毁旧图表实例
- [ ] **跨浏览器兼容性测试**：Safari/Firefox/Chrome 渲染一致性

### 2. GUI 重新设计
- [ ] **设计语言升级**：从"功能堆砌"到"产品级体验"
  - 统一的卡片圆角、阴影、间距系统
  - 更精致的配色方案（不只是黑/白两色）
  - 动效：页面切换、数据加载、图表缩放的过渡动画
- [ ] **导航重构**：顶部 Tab 导航（Dashboard / 因子实验室 / 回测 / 设置）
- [ ] **侧边栏**：自选股快速切换 + 最近搜索
- [ ] **响应式优化**：移动端手势支持（捏合缩放图表）
- [ ] **骨架屏优化**：每个区域独立加载，不阻塞整体

### 3. i18n 国际化
- [ ] **完整中文翻译**：覆盖所有 UI 文本（目前部分翻译缺失）
- [ ] **翻译文件外置**：JSON 文件管理，方便后续扩展日语等
- [ ] **数字格式本地化**：中文环境显示 `万`/`亿` 而非 `K`/`M`/`B`
- [ ] **日期格式本地化**：`2024年6月24日` vs `Jun 24, 2024`
- [ ] **货币符号适配**：`$` / `¥` / `€` 根据市场自动切换

---

## 🧠 LLM 智能化升级

### 4. 预测时机优化
- [ ] **盘后预测（16:00+）**：针对下一交易日
  - 使用当日完整日线 + 分时数据
  - 分析当日新闻 + 财报影响
  - 输出：明日限价单价格 + 止损 + 止盈
- [ ] **盘中预测（9:30-16:00）**：针对当日交易
  - 使用实时分时数据（WebSocket 推送）
  - 分析盘中异动 + 资金流向
  - 输出：当日日内交易建议 + 目标价
- [ ] **盘前预测（4:00-9:30）**：针对开盘策略
  - 使用盘前交易数据 + 隔夜新闻
  - 分析期货指数 + 全球市场
  - 输出：开盘策略 + 关键价位

### 5. LLM 深度应用
- [ ] **智能复盘**：每日收盘后自动生成持仓股票的复盘报告
- [ ] **异常检测**：LLM 分析异动原因（为什么暴涨/暴跌？）
- [ ] **组合优化**：根据风险偏好给出仓位建议
- [ ] **新闻深度解读**：不只是情绪分析，而是"这条新闻对我的持仓意味着什么"
- [ ] **财报解读**：自动分析财报数据，给出投资建议
- [ ] **多空博弈分析**：同时给出看多和看空的完整论据
- [ ] **LLM 交易日志**：记录每次预测的依据，用于回测验证

---

## 🧪 因子实验室重构

### 6. 因子实验室 — 易用性
- [ ] **向导模式**：引导式因子构建（选择类型 → 选择参数 → 测试 → 保存）
- [ ] **因子模板库**：按策略分类（动量/均值回归/突破/趋势跟随）
- [ ] **一键测试**：点击模板直接在当前股票上运行
- [ ] **因子参数优化**：自动搜索最优参数组合
- [ ] **因子可视化增强**：
  - 因子值与价格的双轴对比图
  - 信号标记点（买入/卖出信号在 K 线上的标注）
  - 因子分布直方图 + 正态拟合
- [ ] **因子回测集成**：因子评估结果直接关联到回测面板

### 7. 因子实验室 — 进阶功能
- [ ] **多因子组合构建器**：拖拽式组合多个因子
- [ ] **因子有效性检验**：IC 分析 + 分组回测 + 衰减分析
- [ ] **因子正交化**：去除因子间共线性
- [ ] **因子中性化**：去除行业/市值暴露
- [ ] **自定义因子注册**：保存、分享、导出因子表达式

---

## 📊 回测系统增强

### 8. 回测 — 用户体验
- [ ] **策略模板库**：预置经典策略（双均线、RSI 均值回归、MACD 趋势等）
- [ ] **回测参数可视化**：参数调整时实时预览收益曲线变化
- [ ] **回测报告导出**：PDF/CSV 格式
- [ ] **多股票回测**：在一组股票上批量测试策略
- [ ] **基准对比**：与 SPY/QQQ 的收益对比曲线

### 9. 回测 — 高级功能
- [ ] **Walk-Forward 优化**：滚动窗口参数优化
- [ ] **Monte Carlo 置信区间**：收益分布可视化
- [ ] **风险指标增强**：Sortino 比率、Calmar 比率、最大回撤持续期
- [ ] **交易成本建模**：滑点模型 + 手续费阶梯

---

## 🔮 未来方向

### 10. 数据增强
- [ ] **多数据源容灾**：yfinance 失败时降级到 Alpha Vantage / Twelve Data
- [ ] **SEC 文件解析**：自动提取 8-K/10-Q 关键信息
- [ ] **分析师评级**：目标价 + 评级变化
- [ ] **内部交易**：高管买卖记录
- [ ] **期权链分析**：隐含波动率 + Put/Call 比率

### 11. 框架迁移（长期）
- [ ] **SvelteKit 迁移**：组件化重构，提升开发效率
- [ ] **TypeScript**：类型安全
- [ ] **组件库**：Shadcn-svelte
- [ ] **状态管理**：Svelte stores
- [ ] **E2E 测试**：Playwright

### 12. 生产化
- [ ] **用户认证**：多用户支持
- [ ] **云端部署**：数据库迁移到 PostgreSQL
- [ ] **性能优化**：懒加载、虚拟滚动
- [ ] **监控告警**：API 响应时间、错误率追踪

---

## 实施优先级

```
P0 (立即)  → 图表稳定性 + i18n 完善
P1 (近期)  → GUI 重新设计 + LLM 预测时机优化
P2 (中期)  → 因子实验室重构 + 回测增强
P3 (长期)  → 框架迁移 + 生产化
```

---

## 文件结构

```
backend/          22 Python 模块
frontend/         1 SPA (index.html)
tests/            5 测试文件, 66 tests
docs/archive/     旧版路线图归档
```
